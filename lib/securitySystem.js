...
class SecuritySystem {
    ...
    getBatteryLevel(callback) {
        ...
        callback(null, state ? state.alarm.batteryLevel : state);
    }

    getLowBatteryStatus(callback) {
        this.log('Battery status requested');
        let state = this.adt.getState();
        callback(null, state ? state.alarm.lowBatteryStatus : state);
    }

    getCurrentState(callback) {
        this.log('Current state requested');
        let state = this.adt.getState();
        callback(null, state ? state.alarm.armingState : state);
    }

    getTargetState(callback) {
        this.log('Target state requested');
        let state = this.adt.getState();
        callback(null, state ? state.alarm.targetState : state);
    }
