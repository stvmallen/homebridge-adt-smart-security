    ...
    async getStatusFromDevice() {
        ...
        await request(options)
            .then((response) => {
                ...
                let alertSign = $('.dashboardDevice .orangeWarningMessage');
                let deviceInactive = $('.dashboardDevice .deviceInactive');
                ...
                if (deviceInactive.length > 0) {
                    state.alarm.faultStatus = 1;
                }

                if (alertSign.length > 0 && deviceInactive.length === 0) {
                    this.log.warn('Alarm triggered!!');
                    state.alarm.armingState = 4; // TRIGGERED
                }
        ...
        state.cameras.forEach(camera => this.getImage(camera.id));
