...
class Adt extends events.EventEmitter {
    ...
    async init() {
        try {
            ...
            this.statusCache.on('del', (key, value) => {
    ...
    async getState() {
        ...
        if (!cachedStatus) {
            ...
            cachedStatus = await pEvent(this.statusCache, 'set', {multiArgs: true, timeout: 20000})[1];
        }
    ...
    async attemptToRecoverFromFailure() {
        try {
            ...
            await this.login();
            let newState = await this.getStatusFromDevice();
            this.statusCache.set(STATUS, newState);

            this.log.info("Recovered from error");
        } catch (error) {
            this.log.warn("Still failing", error);
