    ...
    async getCurrentStatus() {
        ...
        await request(options)
            .then((response) => {
                ...
                if (!this.targetState) {
                    this.targetState = state.alarm.armingState;
                } else if (this.targetState !== state.alarm.armingState && !this.changingState) {
                    this.log.debug("Changing target state from %s to %s", this.targetState, state.alarm.armingState);
                    this.targetState = state.alarm.armingState;
                }
    ...
    async changeState(mode) {
        ...
        await request(options)
            .then((response) => {
                ...
                viewState = $('input[type=hidden][name=javax\\.faces\\.ViewState]').val();

                switch (mode) {
                    case 0:
                        action = $('#activationButtons li.center a[title]').attr('id');
                        break;
                    case 1:
                        action = $('#activationButtons li.right a[title]').attr('id');
                        break;
                    case 3:
                        action = $('#activationButtons li.left a[title]').attr('id');
                        break;
                    default:
