...
FFMPEG.prototype = {
    ...
    handleStreamRequest(request, retry = 0) {
        let sessionID = request["sessionID"];
                        ...
                        .then(videoSource => {
                            ...
                            ffmpegSession.on('close', function (code) {
                                if (code == null || code === 0 || code === 255) {
                                    ...
                                    if (retry < 3) {
                                        that.log.warn('Retrying stream');
                                        that.handleStreamRequest(request, 1 + retry);
                                        that.pendingSessions[sessionIdentifier] = sessionInfo;
                                    }
                                }
                            });
